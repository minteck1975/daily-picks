...
1. **Filters for established uptrends** — price > MA20 > MA50 with MA20 rising
1. **Enriches the top ~12 candidates** with sentiment, insider data, weekly trend, earnings calendar, and sector classification
1. **Selects up to 3 picks** — at most one per sector, none with earnings in the next 3 days, none with weekly-down trends
1. **Computes a full trade setup per pick** — ATR-based stop, nearest-resistance target, risk-reward ratio
1. **Snapshots the broader regime** — sector breadth, VIX, junk-credit (HYG), risk-on/off label
1. **Logs picks to a persistent track record** that auto-computes returns at +1 / +5 / +30 days
1. **Commits everything back to the repo**, triggering a Pages redeploy
...
```
┌─────────────────────────────────────┐
│  GitHub Actions cron (5:21am SGT)   │
...
.
├── .github/workflows/
│   ├── daily-picks.yml          # cron job (5:21am SGT, post-US-close)
...
Cron line in `daily-picks.yml`. **Times are always UTC** — convert from your local time.

|Time you want                   |Cron           |Notes                                               |
|--------------------------------|---------------|----------------------------------------------------|
|**5:21 AM SGT (after US close)**|`21 21 * * 1-5`|**Current setting.** Off-minute dodges GH load spike|
|5 AM SGT (exact, after US close)|`0 21 * * 1-5` |Top-of-hour — may be delayed                        |
|5 PM SGT (after SGX close)      |`0 9 * * 1-5`  |Previous setting                                    |
|6 AM SGT (post-close + buffer)  |`0 22 * * 1-5` |Extra hour for EOD data to settle                   |

Singapore is UTC+8 (no DST). 5 AM SGT = 21:00 UTC the prior day. The US market
closes 4 PM ET = 20:00–21:00 UTC depending on US daylight saving, so a 21:00 UTC
run lands right at or just after the close with fresh end-of-day bars.
